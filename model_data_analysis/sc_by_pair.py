...
for dir_name in dir_list:
    ...
    for year in years:
        ...
        if os.path.exists(os.path.join('/data_dustin/store3/training/matt/self_citation/results_12_2022/', journal_name + str(year) + '.csv')):
            print('Skipping because file exists: ' + os.path.join('/data_dustin/store3/training/matt/self_citation/results_12_2022/', journal_name + str(year) + '.csv'))
            continue
        ...
        for entry_idx, this_eid in tqdm(enumerate(EIDs)):       
            ...
            for ref_idx, ref_full in enumerate(ab_full.references):
                ...
                if ref_full.id in missing_ref_eid:
                    missing_key = True
                    try:
            ...
            except Scopus404Error:  # in this case, if first or last author is missing, skip both
                continue
            except Scopus500Error:
                continue
            
            ######## author names (more thorough) 
            # first author
        ...
        df_year.to_csv(os.path.join('/data_dustin/store3/training/matt/self_citation/results_12_2022/', journal_name + str(year) + '.csv'),
                                    index=False)
